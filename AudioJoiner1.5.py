...
def gather_metadata():
    ...
    tk.Label(root, text="Track Title:").grid(row=3, column=0, sticky='w', padx=10, pady=5)
    track_title_entry = tk.Entry(root)
    track_title_entry.grid(row=3, column=1, padx=10, pady=5)
    ...
    def on_ok():
        ...
        metadata["track_title"] = track_title_entry.get()
...
def join_audio_files(file_paths, output_file_name, bitrate, metadata):
    ...
    # Set tags using pydub before exporting
    combined_audio.artist = metadata["artist"]
    combined_audio.album = metadata["album"]
    combined_audio.title = metadata["track_title"]

    combined_audio.export(output_file_name, format="mp3", bitrate=bitrate, tags=metadata)

    # Set track_num using eyed3 (since pydub doesn't handle it)
...
if __name__ == "__main__":
    ...
    output_file_name = simpledialog.askstring("Input", "Name your output file:", initialvalue="combined_output.mp3")
